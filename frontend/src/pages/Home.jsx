...
export default function Home() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="mb-10">
        <h1 className="text-3xl font-bold text-slate-800 tracking-tight">Trending Now</h1>
        <p className="text-slate-500 mt-2">Discover our latest collection of premium products</p>
      </div>

      {filteredProducts.length === 0 ? (
        <div className="text-center py-20 bg-white rounded-3xl border border-slate-100 shadow-sm">
          <p className="text-slate-500 text-lg">No products found {searchQuery ? `matching "${searchQuery}"` : ''}</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          ...
          >
            ...
            <div className="p-5 flex-1 flex flex-col">
              ...
              <h3 className="font-semibold text-slate-800 text-lg mb-1 line-clamp-1">{product.name}</h3>
              ...
              <div className="flex items-center justify-between mt-auto pt-4 border-t border-slate-100">
                <div className="flex flex-col">
                  {product.discountedPrice && product.discountedPrice < product.originalPrice ? (
                    <>
                      <span className="text-lg font-bold text-slate-900">₹{product.discountedPrice}</span>
                      <span className="text-sm text-slate-400 line-through">₹{product.originalPrice}</span>
                    </>
                  ) : (
                    <span className="text-lg font-bold text-slate-900">₹{product.originalPrice || '0.00'}</span>
                  )}
                </div>
                <button 
