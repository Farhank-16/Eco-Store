...
import { ShoppingBag, ShoppingCart, Loader2, Star, Truck, ShieldCheck, ArrowLeft } from 'lucide-react';
...
export default function ProductDetails() {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const res = await axios.get(`http://localhost:3000/product/get/${slug}`);
  ...
  if (!product) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh]">
        ...
        <button onClick={() => navigate(-1)} className="mt-4 text-indigo-600 font-medium hover:underline">Go Back</button>
      </div>
    );
  }
          ...
          <div className="mb-8">
            {product.discountedPrice && product.discountedPrice < product.originalPrice ? (
              <div className="flex items-end gap-3">
                <span className="text-4xl font-extrabold text-slate-900">₹{product.discountedPrice}</span>
              ...
              <span className="text-4xl font-extrabold text-slate-900">₹{product.originalPrice || '0.00'}</span>
            )}
            <p className="text-sm text-slate-500 mt-2">Taxes included. Free shipping on orders over ₹500.</p>
          </div>
